...
export default function App() {
  ...
  const handleSelectTimetable = async (id: string) => {
    try {
      await fetch(`/api/timetables/active/${id}`, { method: "POST" });
      loadTimetable(id);
    } catch (err: any) {
      console.error(err);
    }
  ...
  const handleLoadSample = async () => {
    const res = await fetch("/api/sample-timetable", { method: "POST" });
  ...
  const handleSaveRecord = async (updatedFields: Partial<TimetableEntry>) => {
    ...
    if (editingEntry) {
      ...
      const res = await fetch(`/api/records/${newRecord.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleExport = (format: "pdf" | "csv" | "excel") => {
    ...
    if (format === "csv") {
      ...
      window.location.href = `/api/export/csv?timetableId=${timetable.id}${searchParam}`;
    } else if (format === "excel") {
      const searchParam = isFiltered ? `&search=${encodeURIComponent(searchQuery.trim())}` : "";
      window.location.href = `/api/export/excel?timetableId=${timetable.id}${searchParam}`;
    } else if (format === "pdf") {
            ...
            {currentView === "pdf" && (
              <div className="neu-raised p-6 text-center space-y-4">
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <a
                    href={`/api/pdf-view/${timetable.id}`}
