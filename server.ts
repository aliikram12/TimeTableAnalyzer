...
function runPythonEngine(args: string[]): Promise<any> {
  return new Promise((resolve, reject) => {
    const pythonScript = path.join(__dirname, "backend", "engine.py");
    ...
    execFile(PYTHON_BIN, cmdArgs, { maxBuffer: 10 * 1024 * 1024 }, (err, stdout, stderr) => {
      ...
      try {
        ...
        // Parse last JSON line in case of warnings
...
// Helper to query SQLite database directly via Python script or helper
function queryDb(action: string, payload: any = {}): Promise<any> {
  return new Promise((resolve, reject) => {
    const backendDir = path.join(__dirname, "backend").replace(/\\/g, "/");
    const script = `
import sys, json, os
